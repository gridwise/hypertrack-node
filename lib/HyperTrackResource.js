...
HyperTrackResource.prototype = {
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    function makeRequest() {
      ...
      RequestLib({
        uri: "https://api.hypertrack.com" + path,
        method: method,
        json: true,
        headers: headers,
        ...
        if (err) {
          callback.call(self, new Error.HyperTrackConnectionError({
              message: 'An error occurred with our connection to HyperTrack',
              detail: error}), null);
        }
        // console.log("err, response, body", err, body, response);
      });
      ...
      //     console.log("Socket request data", requestData);
